...
function toggleStep(a) {
  let el = document.getElementById('box-' + a);
  if (el.checked === true) {
    stepList.push(a);
  } else {
...
function stageCheck(a) {
  let stage = document.getElementById(a);
  let stageId = a.split('-')[1];
  let step = document.getElementsByClassName("stage-" + stageId);
  for (let i = 0; i < step.length; i++) {
    let id = step[i].id.split('-');
...
function executeQueue() {
  if (stepList.length > 0) {
    let el = document.getElementById("button-" + stepList[0]).getAttribute('onclick');
    let func = el.substring(el.indexOf("'") + 1, el.lastIndexOf("'"));
...
function resetCheckmarks() {
  const markList = document.getElementsByClassName('bi');
...
function pauseQueue() {
  let el = document.getElementById('pause-button');
